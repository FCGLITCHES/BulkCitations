...
function authorString(authors?: string[]): string {
    ...
    const lastNames = authors.map(a => {
        // Basic heuristic: take the last word after splitting by space or the first before a comma
        const norm = a.replace(/[^\w\s,-]/gi, '').trim().toLowerCase();
...
function hasLocator(parsed: ParsedReference): boolean {
    return !!(parsed.pages || (parsed as any)['article-number']);
}
...
function memberQualityScore(ref: ConvertedReference): number {
    ...
    const coreSignals = [
        ...
        !!(parsed.pages || (parsed as any)['article-number'] || parsed.volume),
    ];
 ...
 * Mutates the items to attach clusterId, and returns the cluster array.
 */
        ...
        if (cluster.members.length > 1) {
            ...
            const memberDiagnostics = cluster.members.map((m) => {
                ...
                return {
                    ...
                    reasons,
                    referenceType: m.referenceType as any,
            ...
            if (!usedApaSourcePreference) {
                ...
                for (const apa of apaCandidates) {
                    ...
                    const sim = calculateCitationSimilarity(apa.parsedData || {} as ParsedReference, bestMember.parsedData || {} as ParsedReference);
                    if (sim >= 90) {
                        bestMember = apa;
