...
function ScholarPreview({
  ...
  return (
    <>
      ...
      >
        <div className="space-y-1.5">
          <p className="text-xs font-semibold text-slate-900">{bandLabel} at {confidence.score}%</p>
...
function ReportButton({
  ...
  return (
    <>
      <ActionButton variant="ghost" onClick={() => setOpen(true)} className="px-0 py-0 text-[15px] font-semibold text-[#dc2626] hover:bg-transparent hover:text-[#dc2626]">
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2.5">
        ...
        {showDebug && refData.styleDetectionFailed ? <Badge className="border border-amber-200 bg-amber-50 text-amber-700" title="The parser fell back to a best guess, so type and field mapping may be less reliable.">Auto-detect uncertain</Badge> : null}
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">Debug view for citations currently flagged as review or action needed.</p>
              </div>
              <Badge className="w-fit border border-slate-200 bg-slate-50 text-slate-600">{visibleReferences.filter((ref) => healthById[ref.id]?.state !== "clean").length} flagged in view</Badge>
            </div>

            <div className="mt-3 flex flex-wrap gap-2">
              {stageDebugSummary.length > 0
                ? stageDebugSummary.map((stage) => (
                    <Badge key={stage.key} title={stage.description} className="cursor-help border border-slate-200 bg-slate-50 text-slate-700">
