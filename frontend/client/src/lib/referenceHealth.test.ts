...
describe('reference health regression checks', () => {
  ...
  it('does not review a journal citation just because issue metadata is absent', () => {
    const ref = makeReference({
      parsedData: {
        authors: ['Smith, J.'],
        title: 'Example title',
  ...
  it('treats online-first journal citations with DOI and no locator as clean', () => {
    const ref = makeReference({
      parsedData: {
        authors: ['Smith, J.'],
        title: 'Example online-first title',
        year: '2024',
        journal: 'Example Journal',
        doi: '10.1000/example-online-first',
      },
      confidence: {
        score: 95,
        breakdown: { rules: 95 },
