...
import {
  invalidateAdminReviewQueries,
  removeDeletedReportFromCitationPages,
...
type VenueFieldKey = "journal" | "conferenceTitle" | "bookTitle";

const EDITABLE_FIELDS: Array<{ key: EditableFieldKey; label: string }> = [
  { key: "authors", label: "Authors" },
  { key: "title", label: "Title" },
  { key: "year", label: "Year" },
  { key: "journal", label: "Venue / Journal" },
...
function readVenueOrJournalValue(
  fields?: {
    journal?: string | null;
  ...
  return fields?.journal ?? fields?.conferenceTitle ?? fields?.bookTitle ?? "";
}

function resolveVenueFieldKey(
  report?: CitationReport | null,
  referenceType?: string,
): VenueFieldKey {
  if (referenceType === "conference") return "conferenceTitle";
  if (referenceType === "bookChapter") return "bookTitle";
  if (
    report?.correctedFields?.conferenceTitle ||
    report?.parsedData?.conferenceTitle
  ) {
    return "conferenceTitle";
  }
  if (report?.correctedFields?.bookTitle || report?.parsedData?.bookTitle) {
    return "bookTitle";
  }
  return "journal";
}
...
function initialCorrectedFields(report?: CitationReport | null) {
  ...
  return {
    authors: Array.isArray(report.correctedFields?.authors)
      ? report.correctedFields?.authors
          .map(
            (author) =>
              author.literal ||
          ...
          .join("; ")
      : Array.isArray(report.parsedData?.authors)
        ? report.parsedData.authors.join("; ")
        : "",
    title: report.correctedFields?.title ?? report.parsedData?.title ?? "",
    year:
      report.correctedFields?.year != null
        ? String(report.correctedFields.year)
        : (report.parsedData?.year ?? ""),
    journal:
      readVenueOrJournalValue(report.correctedFields) ||
      readVenueOrJournalValue(report.parsedData),
    ...
    publisher:
      report.correctedFields?.publisher ?? report.parsedData?.publisher ?? "",
    conferenceTitle:
      report.correctedFields?.conferenceTitle ??
      report.parsedData?.conferenceTitle ??
      "",
    bookTitle:
      report.correctedFields?.bookTitle ?? report.parsedData?.bookTitle ?? "",
    referenceType: String(
      report.correctedFields?.referenceType ?? report.referenceType ?? "",
    ),
  };
}
...
export default function AdminReportDetail() {
  ...
  const [duplicateDecision, setDuplicateDecision] =
    useState<CitationReport["duplicateDecision"]>("not_applicable");
  ...
  const { data: report, isLoading } = useQuery<CitationReport>({
    ...
    queryFn: async () => {
      return adminFetch<CitationReport>(`/internal/admin/reports/${id}`);
    },
    enabled: Boolean(id),
  });

  const handleReportMutationError = (error: Error, fallbackTitle: string) => {
    if (error instanceof AdminRequestError && error.statusCode === 409) {
      ...
      toast({
        title: "Report changed elsewhere",
        description:
          "The latest report state has been reloaded. Review the updated data and retry.",
        variant: "destructive",
      });
      return;
    }
  ...
  useEffect(() => {
    ...
    if (report) {
      ...
      setResolvedByCommit(
        report.resolvedByCommit ??
          report.resolutionTrace?.resolvedByCommit ??
          "",
      );
      setResolvedByVersion(
        report.resolvedByVersion ??
          report.resolutionTrace?.resolvedByVersion ??
          "",
      );
    }
  }, [report]);
  ...
  const assignMutation = useMutation({
    mutationFn: async () => {
      return adminFetch<{ report: CitationReport }>(
        `/internal/admin/reports/${id}/assign`,
        {
          method: "POST",
    ...
    onError: (error: Error) => {
      handleReportMutationError(error, "Assignment failed");
    },
  });
  ...
  const commentMutation = useMutation({
    mutationFn: async () => {
      return adminFetch<{ report: CitationReport }>(
        `/internal/admin/reports/${id}/comments`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    onError: (error: Error) => {
      handleReportMutationError(error, "Comment failed");
    },
  });
  ...
  const rejectMutation = useMutation({
    mutationFn: async (reason: string) => {
      return adminFetch<{ report: CitationReport }>(
        `/internal/admin/reports/${id}/reject`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    onError: (error: Error) => {
      handleReportMutationError(error, "Reject failed");
    },
  });

  const deleteMutation = useMutation({
    mutationFn: async () => {
      return adminFetch<{ success: true; deletedCount: number }>(
    ...
    onSuccess: () => {
      ...
      invalidateAdminReviewQueries(queryClient);
      if (id) {
        removeDeletedReportFromCitationPages(queryClient, id);
      }
  ...
  const resolveMutation = useMutation({
    mutationFn: async ({
      ...
      const proposedPatternPayload = proposedPattern.regex.trim()
        ? {
            regex: proposedPattern.regex.trim(),
            ...(proposedPattern.replacement.trim()
              ? { replacement: proposedPattern.replacement.trim() }
              : {}),
          }
        : undefined;
      const venueFieldKey = resolveVenueFieldKey(
        report,
        correctedFields.referenceType ||
          targetReferenceType ||
          report?.referenceType,
      );
      const venueOrJournalValue = correctedFields.journal || null;

      return adminFetch<{ report: CitationReport }>(
        `/internal/admin/reports/${id}/resolve`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            ...(proposedPatternPayload
              ? { proposedPattern: proposedPatternPayload }
              : {}),
            proposedStyleFix,
            saveAsTruth,
            correctedFields: {
              authors: correctedFields.authors
                ? correctedFields.authors
                    .split(";")
              ...
              year: correctedFields.year
                ? Number.parseInt(correctedFields.year, 10)
                : null,
              journal: venueFieldKey === "journal" ? venueOrJournalValue : null,
              ...
              conferenceTitle:
                venueFieldKey === "conferenceTitle"
                  ? venueOrJournalValue
                  : null,
              bookTitle:
                venueFieldKey === "bookTitle" ? venueOrJournalValue : null,
              referenceType:
                correctedFields.referenceType ||
                targetReferenceType ||
                report?.referenceType,
            ...
            fieldApproval: Object.fromEntries(
              EDITABLE_FIELDS.map((field) => [
                field.key === "journal" ? venueFieldKey : field.key,
                {
                  approved: Boolean(approvedFields[field.key]),
                  value:
                    field.key === "journal"
                      ? correctedFields.journal || undefined
                      : correctedFields[field.key] || undefined,
                },
              ]),
            ),
    ...
    onError: (err: Error) => {
      handleReportMutationError(err, "Resolution failed");
    },
  });
  ...
  if (!report)
    return (
      <div className="p-8 text-center">
        Report not found.{" "}
        <Link href="/admin/reports" className="text-blue-500">
          Go back
        </Link>
      </div>
    );
  ...
  const confidenceScore =
    typeof report.confidence === "number"
      ? report.confidence
      : typeof (report.confidence as unknown as { score?: unknown } | undefined)
            ?.score === "number"
  ...
  const debugChips = [
    ...(report.engineSnapshot?.validationCodes ?? []).map((value) => ({
      label: value,
      tone: "outline" as const,
    })),
    ...(report.engineSnapshot?.qualityFlags ?? []).map((value) => ({
      label: value,
      tone: "secondary" as const,
    })),
    ...(debugProcessingPath?.fallbacksUsed ?? []).map((value) => ({
      label: value,
      tone: "outline" as const,
    })),
    ...(debugProcessingPath?.partialReasons ?? []).map((value) => ({
      label: value,
      tone: "outline" as const,
    })),
    ...(report.engineSnapshot?.splitContaminationFlags ?? []).map((value) => ({
      label: value,
      tone: "outline" as const,
    })),
  ];
        ...
        <div className="flex items-center gap-4 mb-2">
          <Link href="/admin/reports">
            <Button
              variant="ghost"
              size="sm"
              className="gap-2 text-primary-container bg-surface-container hover:bg-outline-variant/10"
            >
          ...
          {report.source === "auto" ? (
            <Badge
              ...
              title="Automatically queued by the system"
            >
          ...
          <div className="xl:col-span-7 space-y-6">
            <Card className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-[0_18px_40px_-24px_rgba(15,23,42,0.28)] dark:border-slate-700/80 dark:bg-slate-900 overflow-hidden">
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {report.source === "user-edit" && (
                      <p className="text-[9px] text-orange-600 mt-1">
                        This was edited by a user. Verify formatting before
                        saving as truth.
                      </p>
                    )}
                  </div>
                ...
                {report.autoQueueReasons &&
                  report.autoQueueReasons.length > 0 && (
                    <div className="space-y-1.5 pt-2">
                      <Label className="text-xs text-red-600 dark:text-red-400 font-bold">
                        Trigger Reasons
                      </Label>
                      <ul className="list-none space-y-1">
                        {report.autoQueueReasons.map((reason, i) => (
                          <li
                            key={i}
                            className="text-xs bg-red-50 dark:bg-red-950/30 p-1.5 flex items-center gap-1.5 rounded text-red-700 dark:text-red-300"
                          >
            ...
            {showDebugTrace && (
              <Card className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-[0_18px_40px_-24px_rgba(15,23,42,0.28)] dark:border-slate-700/80 dark:bg-slate-900">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>
                    Stage-by-stage diagnostics from the engine snapshot so we
                    can see where the failure likely started.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-3 md:grid-cols-4">
                    <div className="rounded-lg border border-border/60 bg-muted/20 p-3">
                      ...
                      <p className="mt-2 text-sm font-semibold">
                        {report.engineSnapshot?.engineVersion ?? "unknown"}
                      </p>
                    </div>
                    <div className="rounded-lg border border-border/60 bg-muted/20 p-3">
                      <p className="text-[10px] uppercase tracking-wide text-muted-foreground">
                      ...
                      <p className="mt-2 text-sm font-semibold">
                        {report.engineSnapshot?.extractorPath ?? "unknown"}
                      </p>
                    </div>
                    <div className="rounded-lg border border-border/60 bg-muted/20 p-3">
                      <p className="text-[10px] uppercase tracking-wide text-muted-foreground">
                      ...
                      <p className="mt-2 text-sm font-semibold">
                        {debugProcessingPath?.partialResult ? "Yes" : "No"}
                      </p>
                    </div>
                    <div className="rounded-lg border border-border/60 bg-muted/20 p-3">
                      <p className="text-[10px] uppercase tracking-wide text-muted-foreground">
                      ...
                      <p className="mt-2 text-sm font-semibold">
                        {debugProcessingPath?.stagesRun?.length ?? 0}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {debugStageLog.length === 0 ? (
                      <div className="rounded-lg border border-dashed border-border/60 px-4 py-6 text-sm text-muted-foreground">
                        No stage diagnostics were captured for this report
                        snapshot.
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {debugStageLog.map((entry) => (
                          <div
                            key={`${entry.stageId}-${entry.status}-${entry.message}`}
                            className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 p-3"
                          >
            ...
            <Card className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-[0_18px_40px_-24px_rgba(15,23,42,0.28)] dark:border-slate-700/80 dark:bg-slate-900">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Badge
                  variant={confidenceScore > 70 ? "secondary" : "destructive"}
                >
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <span className="font-mono">
                        {report.referenceType || "N/A"}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Authors:</span>{" "}
                      <span className="font-mono break-all">
                        {report.parsedData?.authors?.join("; ") || "None"}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Title:</span>{" "}
                      {report.parsedData?.title || "N/A"}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div>
                      ...
                      {readVenueOrJournalValue(report.parsedData) || "N/A"}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Year:</span>{" "}
                      {report.parsedData?.year || "N/A"}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Locator:</span>{" "}
                      {report.parsedData?.pages ||
                        report.parsedData?.volume ||
                        "N/A"}
                    </div>
                  </div>
                </div>
                  ...
                  <CardHeader className="pb-0">
                    <AccordionTrigger className="py-0 hover:no-underline">
                      <div className="text-left">
                        ...
                        <CardDescription>
                          Open this only when you need the full before-and-after
                          audit trail.
                        </CardDescription>
                      </div>
                    </AccordionTrigger>
                    ...
                    <CardContent className="grid grid-cols-1 xl:grid-cols-3 gap-4 pt-4">
                      <div className="space-y-2">
                        ...
                        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 p-3 text-xs space-y-2">
                          <div>
                            <span className="text-muted-foreground">Type:</span>{" "}
                            {report.originalEngineOutput?.referenceType ??
                              report.referenceType ??
                              "N/A"}
                          </div>
                          <div>
                            ...
                            {report.originalEngineOutput?.confidence ??
                              report.confidence ??
                              "N/A"}
                          </div>
                          <div className="break-words font-mono text-[11px]">
                            {report.originalEngineOutput?.convertedText ??
                              report.convertedText}
                          </div>
                        </div>
                      </div>
                      <div className="space-y-2">
                        ...
                        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 p-3 text-xs space-y-2">
                          {Object.entries(report.correctedFields ?? {})
                            .length === 0 ? (
                            <p className="text-muted-foreground">
                              No corrected fields stored yet.
                            </p>
                          ) : (
                            Object.entries(report.correctedFields ?? {}).map(
                              ([field, value]) => (
                                <div
                                  key={field}
                                  className="grid grid-cols-[110px,1fr] gap-2"
                                >
                                  ...
                                  <span className="break-words">
                                    {Array.isArray(value)
                                      ? value
                                          .map((entry) =>
                                            "literal" in entry && entry.literal
                                              ? entry.literal
                                              : "first" in entry && entry.first
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="rounded-md border border-emerald-200/50 bg-emerald-50/10 p-3 text-xs leading-6">
                          {report.finalApprovedOutput ||
                            ...
                            "No final approved output stored yet."}
                        </div>
                      </div>
                    </CardContent>
                  </AccordionContent>
          ...
          <div className="xl:col-span-5 space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Keep the active review lightweight. The deeper provenance and
                  timeline stay tucked into advanced context.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                <div className="rounded-lg border border-border/60 bg-muted/20 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Badge
                      variant={
                        report.likelyStageBlame &&
                        report.likelyStageBlame.confidence >= 0.8
                          ? "default"
                          : report.likelyStageBlame &&
                              report.likelyStageBlame.confidence >= 0.5
                            ? "secondary"
                            : "outline"
                      }
                    >
                    ...
                    <Badge variant="outline" className="text-[10px]">
                      {report.likelyStageBlame
                        ? `${Math.round(report.likelyStageBlame.confidence * 100)}% confidence`
                        : "No stage blame"}
                    </Badge>
                    {report.assigneeName ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={
                          !assigneeName.trim() || assignMutation.isPending
                        }
                      >
                        Assign
                      </Button>
                      ...
                      <div className="rounded-lg border border-border/60 bg-muted/20 p-4 space-y-3">
                              ...
                              <Badge
                                variant={
                                  ...
                                  report.likelyStageBlame.confidence >= 0.8
                                    ? "default"
                                    : report.likelyStageBlame &&
                                        report.likelyStageBlame.confidence >=
                                          0.5
                                      ? "secondary"
                                      : "outline"
                                }
                              >
                                {report.likelyStageBlame?.likelyStage ??
                                  "unknown"}
                              </Badge>
                              <span className="text-sm font-medium">
                                {report.likelyStageBlame
                                  ? `${Math.round(report.likelyStageBlame.confidence * 100)}% confidence`
                                  : "No stage blame captured"}
                              </span>
                            </div>
                          </div>
                          {report.engineSnapshot?.truthProvenance
                            ?.truthApplied && (
                            <Badge variant="outline" className="text-[10px]">
                              Truth applied
                            </Badge>
                        ...
                        {!!report.likelyStageBlame?.alternatives?.length && (
                          <div className="space-y-1">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {report.likelyStageBlame.alternatives.map(
                                (alternative) => (
                                  <Badge
                                    key={`${alternative.stage}-${alternative.confidence}`}
                                    variant="outline"
                                    className="text-[10px]"
                                  >
                                    {alternative.stage}{" "}
                          ...
                          <div className="mt-2 text-xs">
                            {report.truthId ? (
                              <div className="space-y-1">
                                ...
                                <p className="font-mono break-all text-slate-500 dark:text-slate-400">
                                  {report.truthId}
                                </p>
                              </div>
                            ) : (
                              <p className="text-slate-500 dark:text-slate-400">
                                No truth entry linked yet.
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 p-3">
                          ...
                          <div className="mt-2 text-xs">
                            {report.regressionFixtureId ? (
                              ...
                              <p className="text-slate-500 dark:text-slate-400">
                                No generated fixture linked yet.
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="max-h-64 space-y-2 overflow-y-auto rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/30 p-3">
                          {(report.reviewEvents ?? []).length === 0 ? (
                            ...
                            (report.reviewEvents ?? []).map((event) => (
                              <div
                                key={event.id}
                                className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-900/50 p-2"
                              >
                                <div className="flex items-center justify-between gap-2">
                                  <div className="flex items-center gap-2">
                                    <Badge
                                      variant="outline"
                                      className="text-[10px] uppercase border-slate-200 dark:border-slate-800"
                                    >
                                      {event.type}
                                    </Badge>
            ...
            <Card className="rounded-2xl border border-blue-200/50 dark:border-blue-900/50 bg-white/95 shadow-[0_18px_40px_-24px_rgba(15,23,42,0.28)] dark:bg-slate-900 overflow-hidden">
              <CardHeader className="bg-blue-50/30 dark:bg-blue-950/10 border-b border-blue-100 dark:border-blue-900/30">
                ...
                <CardDescription className="dark:text-blue-200/60">
                  Determine what needs to be fixed to solve this issue.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-4">
                  <div className="space-y-4 rounded-lg border border-border/60 bg-muted/20 dark:bg-slate-900/40 p-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        value={fixType}
                        onValueChange={(v) => setFixType(v as FixType)}
                      >
                    ...
                    {fixType === "type-correction" && (
                      <div className="space-y-2 p-3 bg-pink-50/20 dark:bg-pink-950/20 rounded border border-pink-100 dark:border-pink-900 border-dashed">
                        <Label className="text-xs font-bold text-pink-700 dark:text-pink-300">
                          Corrected Reference Type
                        </Label>
                        <Select
                          value={targetReferenceType}
                          onValueChange={setTargetReferenceType}
                        >
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          The data is parsed correctly, it's just in the wrong
                          category.
                        </p>
                      </div>
                    )}
                    ...
                    {fixType === "dynamic-pattern" && (
                      <div className="space-y-3 p-3 bg-muted rounded border border-dashed text-xs">
                        ...
                        <p className="text-[9px] text-muted-foreground">
                          An export snippet will be generated for source
                          control. Direct file writes stay opt-in.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 p-4 bg-emerald-50/10 dark:bg-emerald-950/10 rounded border border-emerald-200/40 dark:border-emerald-900/40">
                    <div className="space-y-1">
                      <Label className="text-xs font-bold text-emerald-700 dark:text-emerald-300">
                        Field-Level Truth Approval
                      </Label>
                      <p className="text-[10px] text-muted-foreground">
                        Approve corrected fields so they can become trusted
                        training data for ranking and confidence models.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3 pt-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={duplicateDecision ?? "not_applicable"}
                          onValueChange={(value) =>
                            setDuplicateDecision(
                              value as CitationReport["duplicateDecision"],
                            )
                          }
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="confirmed_duplicate">
                              Confirmed duplicate
                            </SelectItem>
                            <SelectItem value="confirmed_unique">
                              Confirmed unique
                            </SelectItem>
                ...
                <div className="space-y-3 pt-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        resolveMutation.mutate({
                          saveAsTruth: true,
                          modeLabel: "Complete resolution and truth save",
                        })
                      }
                    >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      ...
                      onClick={() => {
                        const reason = prompt(
                          "Why mark this as correct? (e.g. output is already valid)",
                        );
                        if (reason !== null)
                          rejectMutation.mutate(
                            reason || "Output is already correct",
                          );
                      }}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const confirmed = window.confirm(
                          "Delete this report from the review queue? This cannot be undone.",
                        );
                        if (!confirmed) return;
              ...
              <CardFooter className="pt-0 flex flex-col items-start gap-2">
                <Button
                  ...
                  onClick={() => {
                    adminFetch(`/internal/admin/reports/${id}/add-to-stress`, {
                      method: "POST",
                    })
                      .then(() =>
                        toast({ title: "Added to stress test corpus" }),
                      )
                      .catch((error: Error) =>
                        toast({
