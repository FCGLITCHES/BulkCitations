...
export function SiteFooter() {
  ...
  const [persona, setPersona] =
    useState<(typeof personaOptions)[number]["value"]>("student");
  ...
  async function handleWaitlistSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(resolveApiUrl("/api/waitlist"), {
        method: "POST",
      ...
      toast({
        ...
        description:
          "We'll keep you posted with thoughtful updates as the product grows.",
      });
    } catch {
      toast({
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-xl">
              ...
              <p className="text-sm sm:text-[15px] leading-6 text-muted-foreground dark:text-secondary-foreground/78 max-w-lg">
                Making academic citations simple and accurate for researchers
                worldwide.
              </p>
              <div className="h-4 sm:h-5" />

              <div className="rounded-2xl bg-muted/50 dark:bg-black/10 px-4 py-4 sm:px-5">
                <h4 className="font-semibold mb-2.5 text-xs uppercase tracking-[0.16em] text-primary/90">
                  Waitlist
                </h4>
                <p className="text-sm leading-5 text-muted-foreground dark:text-secondary-foreground/80 mb-3">
                  Join other students and researchers getting thoughtful
                  updates, smoother workflows, and product improvements made for
                  real academic work.
                </p>
                <form
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:min-w-[520px] lg:max-w-[560px]">
              <div className="rounded-2xl bg-muted/50 dark:bg-black/10 px-4 py-4">
                <h4 className="font-semibold mb-2.5 text-xs uppercase tracking-[0.16em] text-muted-foreground dark:text-secondary-foreground/60">
                  Support
                </h4>
