...
function normalizeJournalLookupKey(value?: string): string {
    return (value ?? "")
        .toLowerCase()
        .replace(/[^\w\s]/gi, "")
        .replace(/\s+/g, " ")
...
export function expandKnownJournalTitle(journal?: string): string | undefined {
    const trimmed = journal?.trim();
    if (!trimmed) return undefined;

    const exact = JOURNAL_ABBREVIATIONS[trimmed];
    if (exact) {
        return normalizeJournalLookupKey(exact) === normalizeJournalLookupKey(trimmed) ? undefined : exact;
    }

    const noPeriods = trimmed.replace(/\./g, "");
    const normalizedNoPeriods = JOURNAL_ABBREVIATIONS[noPeriods];
    if (normalizedNoPeriods) {
        return normalizeJournalLookupKey(normalizedNoPeriods) === normalizeJournalLookupKey(trimmed)
            ? undefined
            : normalizedNoPeriods;
    }

    const normalizedInput = normalizeJournalLookupKey(trimmed);
    for (const [abbreviation, fullTitle] of Object.entries(JOURNAL_ABBREVIATIONS)) {
        if (normalizeJournalLookupKey(abbreviation) === normalizedInput) {
            return normalizeJournalLookupKey(fullTitle) === normalizedInput ? undefined : fullTitle;
        }
    }
...
export function isLikelyJournalAbbreviation(journal?: string): boolean {
    const trimmed = journal?.trim();
    if (!trimmed) return false;

    if (expandKnownJournalTitle(trimmed)) {
        return true;
    }
    ...
    return words.length > 0
        ...
        && !/journal|review|letters|transactions|proceedings/i.test(trimmed);
}
...
export function findKnownJournalTitleInRaw(rawCitation?: string): string | undefined {
    const raw = rawCitation?.trim();
    if (!raw) return undefined;

    for (const [abbreviation, fullTitle] of Object.entries(JOURNAL_ABBREVIATIONS)) {
        if (abbreviation.length < 8 || !/[.\s]/.test(abbreviation)) {
            continue;
        }

        const matcher = new RegExp(`(^|\\b)${escapeRegExp(abbreviation)}(?=\\b|\\s|$)`, "i");
        if (matcher.test(raw)) {
            return fullTitle;
        }
    }
